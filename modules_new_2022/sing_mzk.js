...
module.exports.run = async function({ event: e, api: t, args: a, body: s }) {
    const n = require("axios"),
        { createReadStream: r, statSync: i, writeFileSync: o, createWriteStream: d, unlinkSync: u } = require("fs-extra"),
        { threadID: c, senderID: l, messageID: g } = e,
        { API_KEY: h } = global.configModule[this.config.name];
    try {
        if (0 == a.length || !a) return t.sendMessage("» Nhập từ cần tìm!", c, g);
        if (0 == a.join(" ").indexOf("https://")) {
            const e = a.join(" ").trim();
            try {
                let { data: a } = await n.get(`https://manhict.tech/video?link=http://youtu.be/${e}&apikey=${h}`);
                ...
                let s = a.title,
                    d = a.link.medium;
                var m = __dirname + `/cache/${rdus}.m4a`;
                const l = (await n.get(d.trim(), { responseType: "arraybuffer" })).data;
                if (o(m, Buffer.from(l, "utf-8")), i(m).size > 1164e6) return t.sendMessage("File cannot be sent because it is larger than 500MB.", c, (() => u(m)), g);
                return await t.sendMessage({ body: s, attachment: r(m) }, c, (() => u(m)), g)
            ...
            var { data: I } = await n.get(`https://manhict.tech/youtube?q=${i}&apikey=${h}`);
            ...
            for (let e in f)
                if (null != f[e].video && (y = f[e].video).duration.length <= timeSearch && "Live" != y.duration) {
                    let e = __dirname + `/cache/${v+=1}.png`,
                        t = `${y.thumbnail_src}`,
                        a = (await n.get(`${t}`, { responseType: "arraybuffer" })).data;
                    o(e, Buffer.from(a, "utf-8")), $.push(r(__dirname + `/cache/${v}.png`)), M = M += 1, p.push(y.id), b += `${M}.《${y.duration}》 ${y.title}\n\n`
                }
    ...
    const s = require("axios"),
        { createReadStream: n, statSync: r, writeFileSync: i, readdirSync: o, unlinkSync: d } = require("fs-extra"),
    ...
    try {
        let { data: o } = await s.get(`https://manhict.tech/singv2/id?id=${a.idYT[e.body-1]}&apikey=${h}`);
        ...
        const m = (await s.get(g, { responseType: "arraybuffer" })).data;
        if (i(f, Buffer.from(m, "utf-8")), r(f).size > 1164e6) return t.sendMessage("File cannot be sent because it is larger than 500MB.", u, (() => d(f)), l);
        return await t.sendMessage({ body: c, attachment: n(f) }, u, (() => d(f)), l)
    } catch (e) { return console.log(e), t.sendMessage("Có lỗi xảy ra:" + e, u, l) }
};